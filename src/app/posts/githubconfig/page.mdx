...
<Header />
<Banner title="Configuração do Git" subtitle="Instalando e configurando git e SSH key"/>
...
<ClipBoard codeText="sudo apt-get update" />
...
<ClipBoard codeText="sudo apt install git" />
...
<ClipBoard codeText='ssh-keygen -t ed25519 -C "your_email@example.com"' />

#### __Passo 2: Precione enter para salvar a chave no local padrão__

![print do terminal com a primeira opção da instalação](/shh-01.png)
...
![print do terminal com a segunda opção da instalação](/shh-02.png)

#### __Passo 4: Se tudo deu certo vai aparece algo parecido com a imagem a seguir:__

![print do terminal com o resultado bem sucedido da instalação](/shh-04.png)
...
<ClipBoard codeText='eval "$(ssh-agent -s)"' />
...
<ClipBoard codeText="ssh-add ~/.ssh/id_ed25519" />
...
<ClipBoard codeText="cat ~/.ssh/id_ed25519.pub" />
...
![Print com o menu selecionado e com um retangolo vermelho em cima da opção Settings](/shh-05.png)

#### Passo 3: No menu lateral que vai aparecer no lado esquerdo da página clique em SSH and GPG keys.

![Print do menu lateral esquedo mostrando o botão "SSH and GPG keys"](/shh-06.png)
...
<ClipBoard codeText='git config --global user.name "Seu nome"' />

#### Passo 2: Subistitua o email de exemplo pelo seu email e precione enter.

<ClipBoard codeText="git config --global user.email emailexemple@exemple.com" />

#### Para saber se foi configurado corretamente você pode digitar os seguintes comando:

<ClipBoard codeText="git config --global user.name" />

&

<ClipBoard codeText="git config --global user.email" />
